...
HOST_SRCS += $(ABS_COMMON_REPO)/harness.c $(ABS_COMMON_REPO)/support.c src/local_support.cpp src/backprop.cpp src/backprop_sda.cpp
...
$(XCLBIN)/$(APP).$(TARGET).$(DSA).xclbin: $(BINARY_CONTAINER_vadd_OBJS)
	mkdir -p $(XCLBIN)
	$(VPP) $(CLFLAGS) --temp_dir $(BUILD_DIR_KERNEL) -R2 -l $(LDCLFLAGS) $(KERNAL_NAME):1 -j 8 -o'$@' $(+) $(XOCC_LINK_OPTS)
...
cleanall: clean
	-$(RMDIR) $(XCLBIN)
	-$(RMDIR) _x.* output.txt .run *.summary
