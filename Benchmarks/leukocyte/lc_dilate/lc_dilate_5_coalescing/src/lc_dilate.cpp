...
void buffer_load_img(int flag, int k, float img_dest[GRID_COLS * (TILE_ROWS + 2)], class ap_uint<LARGE_BUS> *img_src)
{
#pragma HLS inline off
    if (flag) memcpy_wide_bus_read_float(img_dest, (class ap_uint<LARGE_BUS> *)(img_src + (k * TILE_ROWS * GRID_COLS - MAX_RADIUS * GRID_COLS) / (LARGE_BUS / 32)), 0 * sizeof(float), sizeof(float) * ((unsigned long)((TILE_ROWS + 2 * MAX_RADIUS) * GRID_COLS)));
    return;
}
}
...
void workload(class ap_uint<LARGE_BUS> *result, class ap_uint<LARGE_BUS> *img)
{
