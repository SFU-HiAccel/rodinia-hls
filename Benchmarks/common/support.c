...
char* parse_float_array2D(char *s, float *arr, int num_row, int num_col)
{
  int row, col=0;
  char *line, *endptr;
  float data;

  assert(s!=NULL && "Error: parse_float_array2D()::Invalid input string");
  ...
  for (row = 0; row < num_row; ++row)
  {
    ...
    {
      endptr = line;
      data = strtof(line, &endptr);
      
      // *((arr + row*num_col) + col) = data;
      arr[row*num_col + col] = data;
...
int write_float_array2D(int fd, float *arr, int num_row, int num_col)
{
  int row, col=0;
  assert(fd>1 && "Invalid file descriptor");
