...
#pragma HLS pipeline II=1
			...
			float mat_reg_inner_0[TILE_ROWS][SIZE - TILE_ROWS];
			float mat_reg_inner_1[TILE_ROWS][SIZE - TILE_ROWS];
			float mat_reg_inner_2[TILE_ROWS][SIZE - TILE_ROWS];
...
L_kkk:			for (kkk = 0; kkk < chunks_in_inter_row + 2; kkk++) {
				...
				if (kkk % 3 == 0) {
					...
					buffer_compute(compute_flag, kkk - 1, mat_ram_left, mat_reg_inner_2, mat_ram_up, kk);
					buffer_store(store_flag, kkk - 2, mat_reg_inner_1, result, kk);
				}

				else if (kkk % 3 == 1) {
					buffer_load(load_flag, kkk, mat_reg_inner_1, result, kk);
					buffer_compute(compute_flag, kkk - 1, mat_ram_left, mat_reg_inner_0, mat_ram_up, kk);
					buffer_store(store_flag, kkk - 2, mat_reg_inner_2, result, kk);
				}

				else {
					buffer_load(load_flag, kkk, mat_reg_inner_2, result, kk);
					buffer_compute(compute_flag, kkk - 1, mat_ram_left, mat_reg_inner_1, mat_ram_up, kk);
					buffer_store(store_flag, kkk - 2, mat_reg_inner_0, result, kk);
				}
